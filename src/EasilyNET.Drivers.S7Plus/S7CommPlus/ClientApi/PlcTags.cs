...
internal static class PlcTags
{
    public static async Task<int> ReadTagsAsync(this S7CommPlusConnection conn, List<PlcTag> plcTags, CancellationToken ct = default)
    {
        ...
        {
            var logger = S7Log.Instance;
            if (logger?.IsEnabled(LogLevel.Debug) == true)
            {
                logger.LogDebug("ReadTags: Error res={Result}", res);
            }
        }
        return res;
    }
    ...
    public static async Task<int> WriteTagsAsync(this S7CommPlusConnection conn, List<PlcTag> plcTags, CancellationToken ct = default)
    {
        ...
        else
        {
            ...
            if (logger?.IsEnabled(LogLevel.Debug) == true)
            {
                logger.LogDebug("WriteTags: Error res={Result}", res);
            }
        }
        return res;
    }
    ...
    public static PlcTag? TagFactory(string name, ItemAddress address, uint softdatatype, bool Is1Dim = false, ILogger? logger = null, int stringMaxLength = 0)
    {
        switch (softdatatype)
        {
            ...
            default:
                {
                    var log = logger ?? S7Log.Instance;
                    if (log?.IsEnabled(LogLevel.Debug) == true)
                    {
                        log.LogDebug("ERROR: Unknown softdatatype={SoftDataType} for variable={Name}", softdatatype, name);
                    }
                    return null;
                }
        }
